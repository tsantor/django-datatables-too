...
def coverage(c):
    ...
    """
    c.run("coverage run --source django_perm_filter runtests.py tests")
...
def docs(c):
    ...
    c.run("sphinx-apidoc -o docs/ django_perm_filter")
...
def lint(c):
    ...
    """
    c.run("flake8 django_perm_filter tests")
...
def release(c, bumpsize=''):
    ...
    import django_perm_filter
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_perm_filter.__version__))
    c.run("git push --tags")
